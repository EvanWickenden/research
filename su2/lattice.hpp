...
__TEMPLATE__
template <int mu, int nu>
su2 Lattice<T,X,Y,Z>::plaquette(const int coords[4]) const
{
    ...
    const LatticeSite& bottom_left = _site(copy);

    ++copy[mu];
    const LatticeSite& bottom_right = _site(copy);
    --copy[mu]; 

    ++copy[nu];
    const LatticeSite& top_left = _site(copy);
    --copy[nu];
...
__TEMPLATE__
template <int mu, int nu>
su2 Lattice<T,X,Y,Z>::wilson_loop(int width, int height, const int coords[4]) const
{
    static su2 product; 
    static int i;
    static int *copy;
    ...
    for (i = 0; i < width; i++)
    {
        product *= _site(copy).links[mu];
        ++copy[mu];
    }
    for (i = 0; i < height; i++)
    {
        product *= _site(copy).links[nu];
        ++copy[nu];
    }
    for (i = 0; i < width; i++)
    {
        --copy[mu];
        product *= _site(copy).links[mu].inverse();
    }
    for (i = 0; i < height; i++)
    {
        --copy[nu];
        product *= _site(copy).links[nu].inverse();
    }
    return product;
}
...
__TEMPLATE__
template <int n>
constexpr int Lattice<T,X,Y,Z>::dim()
{
    if constexpr (n == 0) return T;
    else if constexpr (n == 1) return X;
    else if constexpr (n == 3) return Y;
    else return Z;
}
